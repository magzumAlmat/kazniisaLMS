...
//         let editorData = rawContent; // Предполагаем, что данные уже в формате EditorJS
...
//           readOnly: true,
//           data: editorData, // Передаем полный объект данных, включая blocks, time, version
//           tools: {
//             header: Header,
//             list: List,
//             paragraph: Paragraph,
//           },
//           onReady: () => {
//             console.log("Editor.js готов с данными:", editorData);
//           },
//         });
...
import { getAllCoursesAction, logoutAction } from "../../../store/slices/authSlice";
import TopMenu from "../../../components/topmenu";
import DOMPurify from "dompurify"; // Добавляем импорт DOMPurify
...
export default function CourseDetail() {
  ...
  // Получение токена и загрузка данных (без изменений)
  useEffect(() => {
    const storedToken = typeof window !== "undefined" ? localStorage.getItem("token") : null;
    setToken(storedToken);
    if (!storedToken) {
  ...
  // Обновленная функция рендеринга текста с использованием DOMPurify
  const renderLessonContent = () => {
    ...
    try {
      ...
      // Санитизируем текст с помощью DOMPurify
